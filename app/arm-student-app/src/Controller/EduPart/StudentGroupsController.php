...
class StudentGroupsController extends AbstractController
{
    ...
    public function index(StudentGroupsRepository $studentGroupsRepository,UserRepository $userRepository): Response
    {
        ...
        /*if ($this->getUser()!=null) {
            ...
            $studentGroupsAccessList = $userRepository->findOneBy(['email' => $user->getUserIdentifier()])->getStaff()->getStudentGroups()->getValues();
            $searchList= array();
            foreach ($studentGroupsAccessList as $item)
            {
                $searchList[]=$item->getId();
            }
            if (in_array('ROLE_CL', $user->getRoles()) and !in_array('ROLE_ROOT', $user->getRoles())) {
                $groupList= $studentGroupsRepository->findBy(['id'=>$searchList]);
